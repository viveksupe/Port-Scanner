...
static multimap<string, std::map<int,string>> nullXmasFinOutputMap;//xmas map
static multimap<string, std::map<int,string>> nullOutputMap;
static multimap<string, std::map<int,string>> FinOutputMap;
static multimap<string, std::map<int,string>> udpOutputMap;
static pthread_mutex_t synMutex = PTHREAD_MUTEX_INITIALIZER;
class TCPUDPScan {
public:
	TCPUDPScan();
	virtual ~TCPUDPScan();
	...
	pthread_mutex_t nullMutex = PTHREAD_MUTEX_INITIALIZER;
	pthread_mutex_t finMutex = PTHREAD_MUTEX_INITIALIZER;
	...
	string checkServices(string ipToScan, int portToScan);
	...
	void nullxmasfinAnalysis(char* packet, int portToScan,
	...
	void sendUDPPacket(int sendSocket, char *buf,
	...
	string serviceName(int port,struct servent * serv);
	string getConclusion(string synOP, string udpOp, string ackOp,
			string XNFOp) ;
