...
// send TCP packet
	...
	if (icmpPacket) {
		...
		nullxmasfinAnalysis(packet, portToScan, DotAddr, 0, flagToSet);
...
string TCPUDPScan::checkServices(string ipToScan, int portToScan) {
	...
	ipToScan.copy(ip, 15, 0);
	checkServ.sin_addr.s_addr = inet_addr(ip);

	if ((portToScan == 80 || portToScan == 22 || portToScan == 24
			|| portToScan == 25 || portToScan == 587 || portToScan == 43
			|| portToScan == 110 || portToScan == 143) ? true : false) {
		...
		if (connect(sockFD, (struct sockaddr *) &checkServ,
		...
		case 80: {
			...
			if (recv(sockFD, getResponse, 1024, 0) > 0) {
				...
				if ((found = recvdResponse.find("Server:")) != string::npos) {
					...
					string recvdResponse(getResponse);
					recvdResponse.copy(buffer, sizeof(buffer),
							found + strlen("Server:") + 1);
					char reutrnversn[25];
					memset(reutrnversn, '\0', 20);
					sprintf(reutrnversn, "HTTP Server: %s", buffer);
					string returnValue(reutrnversn);
					return returnValue;
		...
		case 22: {
			...
			memset(sshResponse, '\0', sizeof(sshResponse));
			int countBytes = recv(sockFD, sshResponse, 40, 0);
			if (countBytes > 0) {
				string returnValue(sshResponse);
				return returnValue;
			}
			break;
		}
		...
		case 587: {
			...
			memset(smtpResponse, '\0', sizeof(smtpResponse));
			char buffer[47];
			memset(buffer, '\0', sizeof(buffer));
			if (recv(sockFD, smtpResponse, 200, 0) > 0) {
				string recvdResponse(smtpResponse);
				if ((found = recvdResponse.find("250")) != string::npos) {
					recvdResponse.copy(buffer, sizeof(buffer),
							found + strlen("250") + 1);
					char reutrnversn[20];
					memset(reutrnversn, '\0', 20);
					sprintf(reutrnversn, "SMTP Server Version: %s", buffer);
					string returnValue(reutrnversn);
					return returnValue;
				} else {
					recvdResponse.copy(buffer, sizeof(buffer),
					...
					char reutrnversn[20];
					memset(reutrnversn, '\0', 20);
					sprintf(reutrnversn, "ESMTP Server Version: %s", buffer);
					string returnValue(reutrnversn);
					return returnValue;
		...
		case 43: {
			...
			memset(whoisResponse, '\0', sizeof(whoisResponse));
			if (recv(sockFD, whoisResponse, 1024, 0) > 0) {
				...
				if (((found = recvdResponse.find("Version")) != string::npos)) {
					...
					string returnValue("Version Not Sent");
					return returnValue;
				}
			}
			break;
		...
		case 110: {
			...
			memset(popResponse, '\0', sizeof(popResponse));
			if (recv(sockFD, popResponse, 200, 0) > 0) {
				...
				if ((found = recvdResponse.find("OK")) != string::npos) {
					...
					char reutrnversn[20];
					memset(reutrnversn, '\0', 20);
					sprintf(reutrnversn, "POP Server Version: %s", buffer);
					string returnValue(reutrnversn);
					return returnValue;
				}
			}
		...
		case 143: {
			...
			memset(imapResponse, '\0', sizeof(imapResponse));
			if (recv(sockFD, imapResponse, 1024, 0) > 0) {
				char buffer[15];
				memset(buffer, '\0', sizeof(buffer));
				string recvdResponse(imapResponse);
				if ((found = recvdResponse.find("]")) != string::npos) {
					recvdResponse.copy(buffer, sizeof(buffer), found + 1);
					char reutrnversn[50];
					memset(reutrnversn, '\0', sizeof(reutrnversn));
...
//init dest soctcpkaddr
	...
	if (icmpPacket) {
		...
		mymap.insert(std::pair<int, string>(portToScan, "(UDP)OPEN"));
		udpOutputMap.insert(
		...
		mymap.insert(std::pair<int, string>(portToScan, "(UDP)OPEN|FILTERED"));
		udpOutputMap.insert(
				pair<string, std::map<int, string>>(DotAddr, mymap));
		pthread_mutex_unlock(&udpMutex);
	}
...
void TCPUDPScan::synAnalysis(char* packet, int portToScan, char* DotAddr,
	...
	if (check == 1) {
		...
		if (tcpheader->ack == 1 && tcpheader->syn == 1) {
			...
			cout << DotAddr << '\t' << portToScan << '\t' << "(SYN)PORT IS OPEN"
					<< endl;
...
void TCPUDPScan::icmpAnalysis(char* packet, char*DotAddr, int portToScan,
	...
	if (strcmp(DotAddr, IP) == 0 && (portFromICMPHeader == portToScan)) {
		if (icmp_header->type == 3
			...
			if ((scanMethod.compare("SYN") == 0)) {
			 ...
			 pair<string, struct displayOutput*>(DotAddr, toOutput));
			...
			else if (scanMethod.compare("XMAS") == 0) {
				pthread_mutex_lock(&xmasMutex);
				mymap.insert(std::pair<int, string>(portToScan, "FILTERED"));
				nullXmasFinOutputMap.insert(
						pair<string, std::map<int, string>>(DotAddr, mymap));
				...
				/*nullXmasFinOutputMap.insert(
				 pair<string, struct displayOutput*>(DotAddr, toOutput));*/
			} else if (scanMethod.compare("NULL") == 0) {
				pthread_mutex_lock(&nullMutex);
				mymap.insert(std::pair<int, string>(portToScan, "FILTERED"));
				nullOutputMap.insert(
						pair<string, std::map<int, string>>(DotAddr, mymap));
				pthread_mutex_unlock(&nullMutex);
				/*nullXmasFinOutputMap.insert(
				 pair<string, struct displayOutput*>(DotAddr, toOutput));*/
			} else if (scanMethod.compare("FIN") == 0) {
				...
				mymap.insert(
						std::pair<int, string>(portToScan, "(ACK)FILTERED"));
				ackOutputMap.insert(
						pair<string, std::map<int, string>>(DotAddr, mymap));
		...
		if ((scanMethod.compare("SYN") == 0)) {
			...
			pthread_mutex_lock(&ackMutex);
			mymap.insert(std::pair<int, string>(portToScan, "(ACK)FILTERED"));
			ackOutputMap.insert(
					pair<string, std::map<int, string>>(DotAddr, mymap));
			pthread_mutex_unlock(&ackMutex);
			/*ackOutputMap.insert(
...
void TCPUDPScan::nullxmasfinAnalysis(char* packet, int portToScan,
		char* DotAddr, int check, string scanMethod) {
	...
	std::map<int, string> mymap;
	/*if (check == 2) {*/
	struct tcphdr * tcpheader = (struct tcphdr*) (packet + ip_header->ihl * 4);
	if (tcpheader->rst == 1) {
	...
	if (check == 3) {
		...
		if (tcpheader->rst == 1) {
			...
			/*toOutput->result = "UNFILTERED";
			 ...
			 pair<string, struct displayOutput*>(DotAddr, toOutput));*/
			pthread_mutex_lock(&ackMutex);
			mymap.insert(std::pair<int, string>(portToScan, "(ACK)UNFILTERED"));
			ackOutputMap.insert(
					pair<string, std::map<int, string>>(DotAddr, mymap));
			pthread_mutex_unlock(&ackMutex);
		} else {
...
/*
 ...
 //sort(synOutputMap.begin(),synOutputMap.end());
 //sort(udpOutputMap.begin(),udpOutputMap.end());
 //sort(nullXmasFinOutputMap.begin(),nullXmasFinOutputMap.end());
 struct servent serv;
 ...
 while (it1 != synOutputMap.end() || it2 != ackOutputMap.end()
 || it3 != udpOutputMap.end() || it4 != nullXmasFinOutputMap.end()) {
 string toOutput = "";
 ...
 + checkServices(ipToPrint, inter1->first) + "\t\t\t"
 + synOP;
 itExtra = it1;
 it1++;
 ...
 for (it2 = ackOutputMap.begin(); it3 != ackOutputMap.end(); it3++) {
 ...
 + checkServices(ipToPrint, inter2->first) + "\t\t\t"
 + ackOp;
 it2Extra = it2;
 it2++;
 } else {
 ...
 + checkServices(ipToPrint, inter3->first) + "\t" + udpOp;
 it3Extra = it3;
 it3++;
 } else {
 toOutput += "";
 ...
 && itExtra != synOutputMap.end()
 && it2Extra != ackOutputMap.end()) {
 for (it4 = udpOutputMap.begin(); it4 != udpOutputMap.end(); it4++) {
 ...
 + checkServices(ipToPrint, inter4->first) + "\t\t\t"
 + XNFOp;
 it4++;
 } else if (it4 != nullXmasFinOutputMap.end()
 && it3Extra == udpOutputMap.end()
 ...
 cout << toOutput + "\t\t" + getConclusion(synOP, udpOp, ackOp, XNFOp)
 << endl;
...
void TCPUDPScan::display(string ipToPrint, struct parseV* parseValues) {
    ...
    while (a != 0) {
        ...
        if (it1 != synOutputMap.end()) {
            ...
            toOutput = to_string(inter1->first) + "\t"
                    + string(serviceName(inter1->first, &serv)) + "\t\t"
                    + checkServices(ipToPrint,inter1->first) + "\t\t\t" + synOP;
            itExtra = it1;
            it1++;
        ...
        if (it2 != ackOutputMap.end() && itExtra != synOutputMap.end()) {
            ...
            toOutput = to_string(inter2->first) + "\t"
                                + string(serviceName(inter2->first, &serv)) + "\t\t"
                                + checkServices(ipToPrint,inter2->first) + "\t\t\t" + ackOp;
...
//          it3++;
        ...
        if (it4 != nullXmasFinOutputMap.end() && it3Extra != udpOutputMap.end()
            ...
            toOutput = to_string(inter4->first) + "\t"
                                + string(serviceName(inter4->first, &serv)) + "\t\t"
                                + checkServices(ipToPrint,inter4->first) + "\t\t\t" + inter4->second;
...
string TCPUDPScan::serviceName(int port, struct servent * serv) {

	if ((serv = getservbyport(htons(port), NULL)) != NULL) {
		string sName(serv->s_name);
		return sName;
	}
	string sName = "UnAssigned";
	return sName;
}
...
string TCPUDPScan::getConclusion(string synOP, string udpOp, string ackOp,
	...
	/*	if (synOP.compare("(SYN)OPEN") == 0 || udpOp.compare("(UDP)OPEN") == 0
	 || udpOp.compare("(UDP)OPEN|FILTERED") == 0
	 || XNFOp.compare("OPEN|FILTERED") == 0) {
	 asd = "OPEN";
	 } else if (synOP.compare("(SYN)CLOSE") == 0
	 || udpOp.compare("(UDP)CLOSE") == 0
	 || XNFOp.compare("CLOSE") == 0) {
	 asd = "OPEN";
	 } else if (udpOp.compare("OPEN")) {
	 asd = "OPEN";
	 }*/
	int filtercount = 0;
	int onlyfiltercount = 0;

	if (XNFOp.compare("CLOSED") == 0) {
		Conclusion = "Closed";
	}
	if (synOP.compare("(SYN)FILTERED") == 0) {
		filtercount++;
	}
	if (udpOp.compare("(UDP)FILTERED") == 0) {
		filtercount++;
	}
	if (udpOp.compare("(UDP)OPEN|FILTERED") == 0) {
		filtercount++;
	}
	if (XNFOp.compare("OPEN|FILTERED") == 0) {
		filtercount++;
	}
	if (XNFOp.compare("FILTERED") == 0) {
		filtercount++;
	}
	if (ackOp.compare("(ACK)UNFILTERED") == 0) {
			filtercount++;
		}
	//Other part
	if (ackOp.compare("(ACK)UNFILTERED") == 0) {
	...
	if (ackOp.compare("(ACK)FILTERED") == 0) {
		onlyfiltercount++;
	}
	if (XNFOp.compare("FILTERED") == 0) {
		onlyfiltercount++;
	}
	if (onlyfiltercount > filtercount) {
		Conclusion = "Filtered";
	}
	if (onlyfiltercount < filtercount) {
		Conclusion = "Open|Filtered";
	}
	if (onlyfiltercount > 0 && filtercount > 0
			&& onlyfiltercount == filtercount) {
		Conclusion = "Filtered";
	}
	if (synOP.compare("(SYN)OPEN") == 0 || udpOp.compare("(UDP)OPEN") == 0)
		Conclusion = "Open";
	else if (synOP.compare("(SYN)CLOSED") == 0
			|| udpOp.compare("(UDP)CLOSED") == 0)
		Conclusion = "Closed";
